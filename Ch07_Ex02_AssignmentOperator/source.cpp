/* @brief Assignment from chapter 7 the Exercise 2.
	...
	default:
		if (isalpha(ch)) {
			...
			cin.putback(ch);	// The last character that does not fullfil the
								// previous conditions needs to be put back, or it
								// will get "lost".
...
double declaration()
{
	...
	if (is_declared(varName))
		...
		varTable.push_back( Variable(varName, d) ); // add new variable
	return d;
}
...
//------------------------------------------------------------------------------
/* @brief clean the silly inputs that might be found in the buffer, in case
 * 			of an error input.
 * */
void clean_up_mess()
{
