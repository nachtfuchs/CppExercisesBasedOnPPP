...
/* @brief Equality is achieved, if certain patron data is identical
 * */
bool operator==( const Patron& patronA, const Patron& patronB){
	return ( patronA.getCardNumber() == patronB.getCardNumber() &&
			patronA.getName() == patronB.getName() );
}

/* @brief Overlaod for better usability
 * */
ostream& operator<<(ostream& os, Patron inPatron)
{
	return os << "Name: " << inPatron.getName() << ',' <<
			" Card number:  " << inPatron.getCardNumber() << ';';
}
} // LibPerson
